...
namespace TP_INTEGRADOR_COPIA
{
        internal class Program
        {
            ...
            //static void StockConProductoVendido(Cerveza[] infoDeCerveza, Cerveza[] Ventas)
            //{

            //}
            //static void Ventas(List<Cerveza[]> cervezas, string productoAVender)
            //{
            //    Console.WriteLine("Seleccione el número del producto que desea vender:");
            //    for (int i = 0; i < cervezas.Count; i++)
            //    {
            //        Console.WriteLine($"Producto {i + 1}: {cervezas[i][0].NombreCerveza}, Presentación: {cervezas[i][0].PresentacionCerveza}, Stock: {cervezas[i][0].StockCerveza}");
            //    }
            //}
            // Nueva función para manejar las ventas
            static void Ventas(List<Cerveza[]> cervezas)
            {
                ...
                if (productoSeleccionado > 0 && productoSeleccionado <= cervezas.Count)
                {
                    ...
                    Console.WriteLine($"Producto seleccionado: {cervezas[productoSeleccionado][0].NombreCerveza}, Presentación: {cervezas[productoSeleccionado][0].PresentacionCerveza}, Stock: {cervezas[productoSeleccionado][0].StockCerveza}");
                    ...
                    if (cantidadVendida > 0)
                    {
                        ...
                        if (cantidadVendida <= cervezas[productoSeleccionado][0].StockCerveza)
                        {
            ...
            static void StockConProductoVendido(List<Cerveza[]> cervezas, int productoSeleccionado, int cantidadVendida)
            {
                // Restar la cantidad vendida del stock
                cervezas[productoSeleccionado][0].StockCerveza -= cantidadVendida;

                // Mostrar el stock actualizado
                Console.WriteLine($"Venta realizada. Nuevo stock del producto {cervezas[productoSeleccionado][0].NombreCerveza}: {cervezas[productoSeleccionado][0].StockCerveza}\n");
                ...
                for (int i = 0; i < cervezas.Count; i++)
                {
                    ...
                    foreach (Cerveza listaCerveza in cervezas[i])
                    {
                        Console.WriteLine($"Estilo: {listaCerveza.NombreCerveza}, " +
                            $"Presentación: {listaCerveza.PresentacionCerveza}" +
                            $" Stock: {listaCerveza.StockCerveza}, " +
                            $"Precio: {listaCerveza.PreciosCerveza}");
